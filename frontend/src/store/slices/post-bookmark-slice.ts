...
export const getUserBookmarks = createAsyncThunk('users/getUserBookmarks', async (_, api) => {
    const dispatch = api.dispatch as AppDispatch;
    await fakeDelay(1000);
    try {
        const _id = getURLSearchParam('userid');

        const { data } = await axios.get(`${CONFIG_API_URL}/api/bookmarks/reviews/${_id}`);
...
export const postBookmark = createAsyncThunk('users/postBookmark', async ({ reviewId, profName }: IName, api) => {
    const dispatch = api.dispatch as AppDispatch;
    await fakeDelay(1000);
    ...
    try {
        ...
        const { data } = await axios.post(`${CONFIG_API_URL}/api/bookmarks/reviews/${userId}/${reviewId}`, body);
...
export const BookmarkSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(postBookmark.fulfilled, (state, action) => {
            return {
                ...state,
                data: action.payload.bookmarkedReviews,
                loading: false,
            };
        });
        ...
        builder.addCase(getUserBookmarks.fulfilled, (state, action: PayloadAction<IBookmark[]>) => {
            return {
                ...state,
                data: action.payload,
                loading: false,
